...
Der Code funktioniert nicht, weil die eingebaute Funktion `sum` durch eine Ganzzahl "überschrieben" wird (nämlich mit der Summe, welche mittels Schleife berechnet wird). Danach kann die eingebaute Funktion `sum` nicht mehr aufgerufen werden. Es wäre hier also besser, anstelle von `sum` einen Namen zu verwenden, der keiner eingebauten Funktion entspricht, z.B. `s`:
...
Der Code enthält zwei Fehler: Nach `if a < 10` fehlt der abschließende Doppelpunkt, und der `print`-Aufruf verwendet die (in Python 2 gültige, in Python 3 aber ungültige) Anweisungsform ohne Klammern. Der korrekte Code lautet:
...
for i, line in enumerate(lines, start=1):
    if "thereby" in line:
        print(f'"thereby" gefunden in Zeile {i}.')
```
...
def f(x):
    return (math.exp(2 * x) + math.sin(x)) / math.sqrt(1 + x**2)
...
rng = default_rng(1)
x = rng.integers(low=0, high=101, size=(15, 5))
x.mean(axis=0)
```
...
Der Fehler liegt darin, dass die Funktion `np.zeros` ein Tupel (oder eine Liste) mit den Dimensionen des Arrays erwartet, nicht zwei separate Argumente. Der korrekte Code lautet daher:
