...
```{python}
u.mean(axis=1)  # Zeilenmittelwerte
```

```{python}
u.mean(axis=0)  # Spaltenmittelwerte
```

```{python}
u.max(axis=0)  # Spaltenmaxima
```

```{python}
u.min(axis=0)  # Spaltenminima
```

```{python}
u.max(axis=1)  # Zeilenmaxima
```

```{python}
u.min(axis=1)  # Zeilenminima
```

```{python}
u.mean(axis=1).max()  # Maximum der Zeilenmittelwerte
```
