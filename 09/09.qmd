...
Darüber hinaus gibt es viele weitere Pakete für spezifischere Anwendungen wie z.B. [Scikit-learn](https://scikit-learn.org/) (Machine Learning), [statsmodels](https://www.statsmodels.org/stable/index.html) (Statistik), [scikit-image](https://scikit-image.org) (Bildverarbeitung) und [SymPy](https://www.sympy.org/en/index.html) (symbolisches Rechnen). Viele dieser Pakete bauen auf NumPy auf; daher werden wir es in dieser Einheit näher kennenlernen.


## NumPy

Für umfangreiche numerische Berechnungen sind normale Python-Listen nur bedingt geeignet. Sie können zwar beliebige Datentypen enthalten, benötigen dadurch aber mehr Speicher und viele Berechnungen werden vergleichsweise langsam ausgeführt. Für solche Anwendungsfälle stellt NumPy einen effizienten Datentyp bereit: das homogene multidimensionale (n-dimensionale) Array (kurz ND-Array, ndarray oder einfach Array). Ein Array speichert Werte eines gemeinsamen Datentyps sehr kompakt und NumPy kann viele Operationen effizient ausführen. Homogen bedeutet dabei, dass alle Elemente eines Arrays denselben Datentyp haben (z.B. lauter `float`-Werte). Multidimensional bedeutet, dass ein Array beliebig viele Dimensionen (auch Achsen genannt) besitzen kann. Jedes Element wird daher mit einem Tupel indiziert, das seine genaue Position innerhalb des Arrays beschreibt (dazu später mehr).
...
Beginnen wir mit einem einfachen Beispiel, Details zu den einzelnen Befehlen folgen dann später. Konventionell importiert man NumPy so:

```{python}
import numpy as np
```

Dies bedeutet, dass man das Paket mit `np` ansprechen muss statt mit `numpy` – man spart sich also ein paar Zeichen zu tippen. Wir erstellen nun eine Zahlensequenz aus 15 Zahlen:
...
Beginnen wir nach diesem einführenden Beispiel nun aber von vorne und sehen wir uns an, wie wir unterschiedliche NumPy-Arrays erstellen können.
...
:::{.callout-note}
Ein eindimensionales Array kann man sich wie eine Liste vorstellen, wohingegen ein zweidimensionales Array einer Tabelle entspricht.
:::


### Konstant befüllte Arrays

Im Gegensatz zu Listen, welche dynamisch wachsen können (z.B. mit der `append`-Methode), sollte die Größe von Arrays bereits bei der Erstellung bekannt sein, da das Hinzufügen von Zeilen oder Spalten ineffizient ist. Hierfür gibt es einige praktische Konstruktionen, welche Arrays mit Platzhalterelementen wie z.B. lauter Nullen erzeugen.
...
:::{.callout-important}
Die Funktion `np.zeros` erwartet als erstes Argument die gewünschte Form (Shape) des Arrays. Diese wird als Tupel angegeben, im vorigen Beispiel also `(3, 4)`. Deshalb sind zwei Klammerpaare erforderlich: eines für den Funktionsaufruf und eines für das Tupel. Der Aufruf `np.zeros(3, 4)` funktioniert *nicht*, da `3` und `4` dabei als zwei getrennte Argumente interpretiert werden. Lesen Sie in der Dokumentation nach, welche Bedeutung die ersten beiden Parameter der Funktion haben!
:::

:::{.callout-tip}
Übersichtlicher ist es, wenn man Keyword-Argumente nutzt, also beispielsweise:
...
Analog zur eingebauten Funktion `range` können mit `np.arange` Arrays mit Zahlenfolgen erstellt werden. Hier sind nicht nur ganze Zahlen möglich, sondern auch Dezimalzahlen. Wie gewohnt zählt der letzte Wert (Stopp-Wert) nicht mehr zum Bereich dazu.
...
::: {.callout-tip}
Auch hier sind Keyword-Argumente übersichtlicher:
...
Bei Dezimalzahlen als Schrittweite können durch die begrenzte Genauigkeit von Gleitkommazahlen manchmal unerwartete Ergebnisse auftreten. Wenn die Anzahl der Elemente bekannt ist, ist `np.linspace` daher oft die bessere Wahl. Achtung: Hier zählen sowohl Start- als auch Endwert zum Bereich dazu!
...
In diesem Beispiel initialisieren wir den Generator mit `42` (wir könnten aber auch jede beliebige andere Zahl nehmen). Die Initialisierung bewirkt, dass der Generator danach immer dieselbe reproduzierbare Folge an Zufallszahlen ausspuckt (es bedeutet *nicht*, dass die erste Zufallszahl 42 ist). Lässt man die Initialisierung mit einer konkreten Zahl weg, verwendet NumPy einen nicht-deterministischen Seed. Das bedeutet, dass bei jeder erneuten Ausführung eines Scripts andere Zufallszahlen gezogen werden, was nicht im Sinne der Reproduzierbarkeit von Ergebnissen ist.
...
```{python}
rng.integers(low=-3, high=99, size=(2, 5))  # Ganzzahlen von -3 bis 98
```
...
Sind die zwei Arrays nicht gleich groß, kann NumPy das kleinere Array (oder ein Skalar) entlang kompatibler Achsen für die Operation erweitern – dies nennt man *Broadcasting*. Das folgende Beispiel zeigt eine Multiplikation eines Arrays der Shape `(2, 3)` mit der Zahl 5:
...
Hier wird die Zahl 5 also für jedes Element verwendet, sodass die Operation elementweise durchgeführt werden kann. Im Prinzip ist diese Operation äquivalent zu folgender Schreibweise:

```{python}
b * np.full(b.shape, 5)
```

Sehen wir uns ein weiteres Beispiel für Broadcasting mit dem Array `c` an:

```{python}
c = np.array([1, 2, 3])
```

Wir können nun `c` direkt zu `b` addieren, weil die Dimensionen kompatibel sind: `b` hat 2 Zeilen und 3 Spalten und `c` hat 3 Elemente. Durch Broadcasting wird die Operation nun spaltenweise angewendet:
...
::: {.callout-note}
Bei einem zweidimensionalen Array entsprechen die beiden Achsen den Zeilen (Achse 0) und Spalten (Achse 1). Ein dreidimensionales Array mit der Shape `(3, 4, 5)` besteht aus 3 Blöcken mit je 4 Zeilen und 5 Spalten: Achse 0 beschreibt die Blöcke, Achse 1 die Zeilen und Achse 2 die Spalten. Somit entsprechen immer die letzten beiden Achsen den Zeilen und Spalten.
:::
...
::: {.callout-note}
Ein einzelnes Element eines NumPy-Arrays ist kein Array mehr, sondern ein Skalar (also eine einzelne Zahl). Der NumPy-Datentyp wird dabei beibehalten und entspricht meist nicht den eingebauten Python-Datentypen wie `int` oder `float` (sondern z.B. `numpy.int64` oder `numpy.float64`). Solche Skalare lassen sich in Berechnungen in der Regel wie die entsprechenden Python-Zahlen verwenden. Mit `item()` kann man einen Wert bei Bedarf explizit in einen eingebauten Python-Datentyp umwandeln; normalerweise ist das aber nicht notwendig:
...
```{python}
a.item(0)  # dasselbe über den expliziten Index 0
```
:::
...
```{python}
idx = [2, 7, 10, 10]  # wir wollen die Elemente an diesen Indizes
idx
```
...
Es werden hier also nur jene Elemente herausgegriffen, für die das Indexarray (die Maske) `True` ist.
...
```{python}
c.reshape((1, -1))  # reshape gibt ein neu geformtes Array zurück
```
...
:::{.callout-tip}
Man muss bei `reshape` nicht alle Dimensionen angeben – eine kann man auf `-1` setzen, was bedeutet, dass NumPy die passende Anzahl automatisch bestimmt. Dies ist möglich, da die Anzahl aller Elemente konstant bleiben muss.
:::

Der Unterschied zwischen `resize` und `reshape` ist also, dass `resize` das Array direkt modifiziert und `reshape` ein neues Array-Objekt zurückgibt. Dieses ist nach Möglichkeit eine View und teilt dann den Speicher mit dem ursprünglichen Array.
...
Die Lösungen zu diesen Übungen finden Sie [hier](09-solutions.qmd).
