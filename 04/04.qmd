...
Das Konzept von Namen in Python haben wir bereits in einer der vorigen Einheiten kennengelernt. Es ist unerheblich, auf welches Objekt ein Name verweist, dies kann eine Ganzzahl, eine Funktion, oder jedes beliebige andere Objekt sein. So können wir z.B. auch den Namen `f` erstellen, welcher auf die Funktion `print` verweist:
...
:::{.callout-note}
Wie man Funktionen mit *Argumenten* (insbesondere Standard- und Schlüsselwort-Argumenten) aufruft und was es mit *Gültigkeitsbereichen* auf sich hat, sehen wir uns in der nächsten Einheit (Funktionen II) genauer an.
:::
