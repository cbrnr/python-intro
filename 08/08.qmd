...
::: {.callout-note}
"Umwandeln" ist hier eigentlich nicht ganz korrekt, da die Funktion `str` eine String-Repräsentation des übergebenen Objekts zurückgibt (also ein neues String-Objekt erzeugt). Das ursprüngliche Objekt (z.B. die Zahl `55`) bleibt unverändert bestehen.
:::
...
Nun können die Daten in eine Struktur umgewandelt werden, die sich besser für nachfolgende Berechnungen eignet (mit der Funktion `float` werden die Strings in Dezimalzahlen umgewandelt).
