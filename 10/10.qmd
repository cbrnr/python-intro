...
In Python gibt es eine große Anzahl an Paketen, welche sich auf das Erstellen von Grafiken spezialisiert haben. Eine Übersicht über dieses Ökosystem gibt es [hier](https://rougier.github.io/python-visualization-landscape/landscape-colors.html), die bekanntesten Pakete sind momentan:
...
Damit sich in einer `Figure` mit mehreren `Axes`-Objekten keine Überlappungen ergeben, sollten Sie zum Abschluss folgende Zeile ausführen:
...
:::{.callout-note}
Die Box erstreckt sich vom ersten bis zum dritten Quartile, ihre Höhe entspricht also dem Interquartilsabstand (IQR). Die orange Linie in der Box entspricht dem Median. Die sogenannten Whiskers außerhalb der Box erstrecken sich bis zum 1,5-fachen des IQR über bzw. unter dem ersten bzw. dritten Quartil. Punkte außerhalb dieser Grenzen werden als Ausreißer dargestellt.
:::
