...
In Python gibt es zahlreiche Pakete zur Erstellung von Grafiken. Zu den bekanntesten gehören derzeit:
...
Matplotlib ist eines der ältesten und bekanntesten Grafikpakete für Python. Es bietet sehr umfangreiche Möglichkeiten zur Erstellung und Anpassung von Grafiken und eignet sich besonders für hochwertige statische Abbildungen. Die Bedienung ist teilweise etwas ausführlicher als bei neueren Paketen, dafür lässt sich nahezu jeder Aspekt einer Grafik individuell anpassen. Außerdem bauen mehrere andere Grafikpakete auf Matplotlib auf oder orientieren sich an dessen Konzepten. Ein grundlegendes Verständnis von Matplotlib ist daher sehr hilfreich.

Seaborn erweitert Matplotlib um eine einfachere Schnittstelle für die statistische Datenvisualisierung und stellt zahlreiche zusätzliche Diagrammtypen bereit. Plotly und Bokeh legen ihren Schwerpunkt auf interaktive Grafiken, die sich beispielsweise zoomen oder durch Mausbewegungen erkunden lassen. Altair verfolgt einen deklarativen Ansatz und ermöglicht es, mit vergleichsweise wenig Code sowohl statische als auch interaktive Visualisierungen zu erstellen.

Welches Paket am besten geeignet ist, hängt vom jeweiligen Anwendungsfall ab. Für wissenschaftliche Abbildungen und allgemeine Diagramme ist Matplotlib nach wie vor die am häufigsten verwendete Wahl. Seaborn vereinfacht viele statistische Visualisierungen, während Plotly, Bokeh und Altair besonders dann interessant sind, wenn interaktive Grafiken benötigt werden.

In den folgenden Abschnitten beschäftigen wir uns mit Matplotlib und lernen die Grundlagen zur Erstellung von Grafiken kennen.
...
::: callout-note
Die offizielle Matplotlib-Website beinhaltet einige [Tutorials](https://matplotlib.org/stable/tutorials/index.html). Insbesondere der [Quick Start Guide](https://matplotlib.org/stable/tutorials/introductory/quick_start.html) ist eine gute Ergänzung zu den Unterlagen dieser Einheit.
:::
...
::: callout-important
Um eine mit Matplotlib erstellte Grafik in einem separaten Fenster anzuzeigen, muss man explizit den folgenden Befehl ausführen:

```python
plt.show()
```

Dieses Fenster blockiert aber den Python-Interpreter, d.h. man kann keine weiteren Befehle eingeben, solange das Grafikfenster geöffnet ist. Um dieses Verhalten zu umgehen, kann man im interaktiven Python-Interpreter direkt nach dem Importieren folgenden Befehl eingeben:

```python
plt.ion()
```

Damit werden Grafiken sofort nach ihrer Erstellung angezeigt und blockieren den Python-Interpreter nicht ("ion" steht für "interactive on").
:::


### Erstellung von Grafiken

Um mit Matplotlib Grafiken zu erzeugen, sollte man sich mit einigen Grundbegriffen sowie dem Aufbau einer Grafik vertraut machen, was in der folgenden Abbildung veranschaulicht wird (modifiziert von [hier](https://matplotlib.org/stable/gallery/showcase/anatomy.html)):
...
Für den Anfang benötigen wir nur die zwei Begriffe `Figure` und `Axes` (in der Abbildung rot dargestellt). Eine `Figure` entspricht einer Abbildung, die eine oder mehrere Grafiken (sogenannte `Axes`) enthalten kann.
...
Die Funktion gibt die `Figure` sowie die `Axes` zurück; wir nennen diese `fig` bzw. `ax`. Danach kann man Methoden des `Axes`-Objekts `ax` aufrufen, um die gewünschte Grafik zu erstellen (mehr dazu im nächsten Abschnitt).
...
::: callout-note
Eine Box erstreckt sich vom ersten bis zum dritten Quartil, ihre Höhe entspricht also dem Interquartilsabstand (IQR). Die orange Linie innerhalb der Box entspricht dem Median. Die sogenannten Whiskers erstrecken sich standardmäßig bis zum letzten Datenpunkt, der höchstens das 1,5-Fache des IQR unter dem ersten bzw. über dem dritten Quartil liegt. Punkte außerhalb dieser Grenzen werden einzeln als Ausreißer dargestellt.
:::
...
Möchte man beispielsweise Scatterplots mit überlagerten Regressionsgeraden oder andere komplexere Grafiktypen erstellen, sollte man einen Blick auf seaborn werfen. Mit Matplotlib wäre das zwar prinzipiell auch möglich, aber es wären very viele manuelle Schritte notwendig (z.B. müsste man selbst ein lineares Regressionsmodell rechnen, um dieses dann als Gerade in der Grafik darzustellen).
...
Dieser Datensatz enthält den Verlauf von vier verschiedenen Luftgütewerten über einen Zeitraum von fünf Monaten.

::: callout-note
Einige Werte im Datensatz fehlen und werden beim Einlesen als `nan` (*not a number*) gespeichert. Das ist für die folgenden Grafiken kein Problem: Matplotlib lässt diese Werte an den betroffenen Stellen aus.
:::
...
Weisen Sie jeder Spalte eine eigene Variable zu. Verwenden Sie dafür die Namen `ozone`, `solar`, `wind`, `temp`, `month` und `day`, und verwenden Sie diese Variablen in allen folgenden Übungen.
...
Stellen Sie die folgenden vier Grafiken in einer Abbildung dar. Verwenden Sie dazu eine Anordnung aus zwei Zeilen und zwei Spalten:

1. Ein Scatterplot von `ozone` gegen `solar`.
2. Eine Liniengrafik mit dem Verlauf von `temp`.
3. Ein Violinenplot mit den Werten von `temp` für die fünf Monate. Die Werte für diese Monate entsprechen den folgenden Slices: `temp[:31]`, `temp[31:61]`, `temp[61:92]`, `temp[92:123]`, `temp[123:]`.
...
Die Lösungen zu diesen Übungen finden Sie [hier](10-solutions.qmd).
