...
Python ist eine universelle Programmiersprache, welche für viele verschiedene Anwendungen eingesetzt werden kann. Im Gegensatz dazu gibt es spezialisierte Programmiersprachen wie z.B. [R](https://www.r-project.org/), welche für sehr spezifische Aufgaben entwickelt wurden. Beide Ansätze haben Vor- und Nachteile: Spezialisierte Programmiersprachen erlauben es oft, bestimmte Aufgaben schneller, besser oder eleganter zu lösen als universelle Programmiersprachen. Universelle Programmiersprachen sind dafür vielseitiger einsetzbar, was auch den Vorteil hat, dass man nicht unbedingt mehrere Programmiersprachen erlernen muss. Python deckt nicht nur Datenanalyse und Statistik ab, sondern auch komplett andere Anwendungsfälle wie z.B. Webapplikationen, das Erstellen von Programmen mit grafischen Benutzeroberflächen oder auch das Programmieren des [Raspberry Pi](https://www.raspberrypi.org/).
...
Für die Installation unter Windows wählen Sie die Standardinstallation (*"Install Now"*) aus, aber setzen Sie unbedingt das Häkchen bei *"Add python.exe to PATH"*. Falls Sie keine Administratorrechte haben, können Sie hier auch das Häkchen bei *"User admin privileges when installing py.exe"* entfernen. In diesem Fall wird Python nur für den aktuellen Benutzer installiert. Die folgende Abbildung zeigt die empfohlenen Einstellungen:
...
:::{.callout-note}
In den Kursunterlagen wird Python-Code immer in grauen Kästchen dargestellt. Der Prompt `>>>` wird dabei weggelassen, da er ohnehin nicht Teil des Python-Codes ist.
:::
...
Wenn man mathematische Funktionen wie beispielsweise Sinus oder Cosinus verwenden möchte, muss man zuerst das `math`-Modul importieren (aktivieren). Dazu tippt man den folgenden Befehl ein:
