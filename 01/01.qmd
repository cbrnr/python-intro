...
:::{.callout-note}
In diesen Unterlagen wird die Installation der Version 3.10.7 gezeigt. Sie sollten aber die aktuellste Version installieren und verwenden!
:::

Unter Windows wählt man im Installationsfenster die Standard-Installation aus, aber setzt unbedingt das Häkchen bei *"Add Python 3.10 to PATH"*. Die Option *"Install launcher for all users (recommended)"* kann deaktiviert werden, damit können Sie Python auch ohne Administratorrechte installieren. Das folgende Bild zeigt diese Einstellungen:
...
Es gibt einige Möglichkeiten, den Python-Interpreter zu verwenden – mehr dazu in der nächsten Einheit. Zunächst ist es am einfachsten, wenn Sie unter Windows das Programm "Python 3.10 (64-bit)" starten (es befindet sich im Startmenü im Ordner "Python 3.10"). Daraufhin öffnet sich ein Fenster, das in etwa so aussieht:
