...
- Easy to learn
- Open-source (free software, more than just free of charge)
- Cross-platform (runs on Windows, macOS, Linux)
- Versatile (not limited to a single area like e.g. data analysis)
- Suitable for reproducible data analysis
- Extensive standard library
- Large, helpful community
- A huge variety of available third-party packages


## Popularity

Python has become one of the most popular programming languages in the world. Various metrics that gauge the popularity of programming languages, such as [PYPL](https://pypl.github.io/PYPL.html), [IEEE Spectrum Top Programming Languages](https://spectrum.ieee.org/top-programming-languages/), and [TIOBE](https://www.tiobe.com/tiobe-index/), consistently place Python among the top spots, often ranking it first.
...
You can find everything you need to install Python on the [official Python website](https://www.python.org/), where you can download installation packages for various operating systems. The site also hosts Python's extensive official documentation, filled with examples and tutorials. Installing Python on Windows and macOS has become so straightforward that I highly recommend using the official installers (though for Linux, it's still better to use the system's package manager if Python isn't already installed).

:::{.callout-note}
These instructions use version 3.12.2, but you should always install and use the latest version.
:::

On Windows, choose the standard installation, but make sure to check the box for *"Add python.exe to PATH"*. You can leave the option *"Install launcher for all users (recommended)"* unchecked, allowing you to install Python without administrator rights. The image below shows these settings:
...
After installing Python, you can start the so-called Python interpreter. This program processes Python commands and allows you to work interactively (meaning you can type a command, press Enter, and see the result right away).

There are several ways to use the Python interpreter, which we'll cover in the next session. For now, the easiest option is to launch "Python 3.12 (64-bit)" from the Start menu under the "Python 3.12" folder on Windows.

On macOS or Linux, open a terminal and type `python3`, followed by the <kbd>Enter</kbd> key. Make sure the interpreter displays the correct Python version.

In the interactive interpreter, you can start typing Python commands and press Enter to execute them. The prompt `>>>` indicates that Python is ready for input (this prompt is *not* part of the command). You can use the interactive Python interpreter as a calculator, allowing you to perform the four basic arithmetic operations (addition, subtraction, multiplication, and division) like this:
