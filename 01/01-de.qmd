...
- Open Source (freie Software, mehr als nur kostenlos)
...
Unter macOS sollte man nach der Installation noch die beiden Befehle "Install Certificates" und "Update Shell Profile" ausführen:
...
:::{.callout-tip}
Aus Gründen der besseren Lesbarkeit sollten Leerzeichen vor und nach dem Operator eingefügt werden, also besser `10 - 7` und nicht `10-7`. Für Python ist zwar beides korrekt, aber die erste Variante ist leichter lesbar.
:::
...
Wenn Sie mathematische Funktionen wie beispielsweise Sinus oder Cosinus verwenden möchten, müssen Sie zuerst das `math`-Modul importieren (aktivieren), in dem Sie (einmalig) die folgende Zeile eintippen:

```{python}
import math
```

Danach stehen viele mathematische Funktionen wie z.B. `sqrt` (Wurzel), `log` (Logarithmus), `sin` (Sinus), `cos` (Cosinus) und Konstanten wie `pi` (die Kreiszahl $\pi$) oder `e` (die [Eulersche Zahl](https://de.wikipedia.org/wiki/Eulersche_Zahl)) zur Verfügung, die man allesamt mit einem vorangestellten `math.` verwendet:
...
Python wird heutzutage in vielen verschiedenen Bereichen verwendet. Um Ihnen zu verdeutlichen, wie vielfältig man Python vor allem in der Forschung einsetzen kann, sind hier einige relevante Beispiele angeführt. Diese Auswahl ist jedoch bei weitem nicht vollständig und spiegelt vor allem meine persönliche Erfahrung wider.
