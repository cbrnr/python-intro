...
Python ist eine dynamisch typisierte Sprache. Das bedeutet in der Praxis, dass erst zur Laufzeit eines Programms überprüft wird, ob eine bestimmte Operation auf ein Objekt eines bestimmten Typs angewendet werden kann. Praktisch heißt das, dass in Python Typen nicht explizit angegeben werden müssen.
...
Erwartungsgemäß ist `max` jetzt ein Name, der auf das `int`-Objekt 4 verweist. Die ursprüngliche eingebaute Funktion `max` kann man jetzt nicht mehr verwenden:
...
Ein Ausdruck (engl. Expression) ist eine Kombination von Werten, Variablen und Operatoren. Jeder Ausdruck hat einen Wert. Beispiele:
...
Python versucht immer, einen Ausdruck auf einen einzigen Wert zu reduzieren. Beachten Sie außerdem, dass Python im interaktiven Modus Werte automatisch ausgibt – in IPython zu sehen an den `Out[.]`-Zeilen.


## Anweisungen

Eine Anweisung (engl. Statement) ist eine Einheit Code, die der Python-Interpreter ausführen kann. Ein Ausdruck ist eine (spezielle) Anweisung, die einen Wert hat. Beispiele von Anweisungen, die keine Ausdrücke sind:
...
:::{.callout-note}
Kein Wert entspricht in Python eigentlich dem speziellen Wert `None`.
:::
...
Aufgrund der begrenzten Rechengenauigkeit, mit denen Computer Kommazahlen darstellen, kann es zu Rundungsfehlern kommen (Kommazahlen können im Allgemeinen nicht exakt repräsentiert werden). Beispiel:
...
:::{.callout-note}
Statt Kommazahlen mit `==` zu vergleichen ist es besser, wenn man überprüft, ob der Unterschied zwischen den beiden Zahlen sehr klein ist. Dazu gibt es im `math`-Modul eine eigene Funktion `math.isclose`, die man wie folgt verwendet:
...
Sequenzdatentypen können aus mehreren Elementen bestehen. Folgende wichtige Sequenzdatentypen werden von Python unterstützt:
...
*Hinweis:* Sie können die Zahl $\pi$ als `math.pi` verwenden, wenn Sie folgende Zeile ganz am Anfang Ihres Scripts hinzufügen:

```python
import math
```

Die Formel für die Oberfläche $A$ lautet:

$$A = 4 \pi r^2$$

Das Volumen $V$ kann man mit folgender Formel berechnen:
