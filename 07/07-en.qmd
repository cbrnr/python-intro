...
Since lists are *mutable*, you can change individual elements after creating the list.
...
In contrast to strings, the list is *mutated in-place* here. (Re)assigning to a name does not work, as shown in the following example:
...
If you want to add multiple elements at once, you can use the `extend` method:
...
Alternatively, you can use the `pop` method to remove an element at a specific index. This method also returns the removed element (which `del` does not).
...
The `remove` method removes the first element from the list that matches the specified value. Here, you do not specify the index to remove but the value to delete.
...
If a list contains sortable elements (e.g., only numbers), you can sort it with the `sort` method.
...
You can reverse a list (just like a string) with the following slice (this creates a new list since the original list does not change):
...
Just like any sequence data type, a list is iterable, meaning you can iterate over the individual elements with a loop:
...
Tuple behave like lists but are *immutable*. This means that you cannot change the elements contained in the tuple. Tuples are created like lists, but without square brackets (you also do not need parentheses, although these are sometimes syntactically necessary in combination with other commands, e.g., when passing a tuple as an argument).
...
So omitting the comma does not create a tuple but a string:
...
::: {.callout-tip}
To be clear, list comprehensions are *not* a new feature in Python but just a more concise way to create a list. They are often for relatively simple lists, because they are more readable and shorter than the corresponding loop.
:::
...
This is impractical for numerical applications. In one of the following sessions, we will learn how to use Python for numerical calculations (utilizing a new data type called NumPy array, provided by the NumPy third-party package).
:::
...
In summary, list comprehensions are useful as long as they are still relatively simple and clear. If you have several nested loops and conditions, it is better to use explicit loops for better readability.


## Dictionaries

The dictionary data type is a so-called mapping data type. It works similar to a language dictionary. For example, if you want to know the translation of a specific word, you look it up under the word (called a key) and find the translation there (called a value). In Python, you define a `dict` with curly braces and separate the entries with commas. Each entry consists of a key and a value, separated by a colon.
...
Alternatively, you can also use the `dict` function, using keyword arguments to initialize the dictionary:
...
We can access individual elements with indexing, but instead of a numerical index (as with lists), we now specify the respective key as an index:
...
:::{.callout-important}
An essential difference to lists is that dictionaries are *unordered*. Therefore, you cannot access elements by numerical indices (as with lists), but only by keys!
:::

Just like in lists, you can store elements with different data types in dictionaries. However, *keys* must be *immutable* (hence, you cannot use lists as keys). You can add new elements to an existing dictionary by specifying a key and a value:
...
Since Python always evaluates the right-hand side of an assignment first, you can also swap two values with this pattern:
...
If you want to avoid the error when accessing a non-existent key, you can use the `get` method instead. This method has two parameters: a key and a default value that is returned if the specified key does not exist in the dictionary:
...
In the previous session, we defined a function called `find` that behaves like the string method `str.find`. However, this function (and the method) only finds the *first* occurrence of the searched character, e.g., `"Werkstatttreppe".find("t")` returns `5` (the position of the first `t`). It would be useful if the indices of *all* searched characters were returned in a list, e.g., `[5, 7, 8, 9]`. If the character does not occur at all, an empty list should be returned. Write a function `find` that behaves accordingly!
