...
Wie im vorigen Kapitel eingeführt, werden Methoden direkt auf ein Objekt angewandt. Für Strings gibt es in Python sehr viele praktische spezielle Funktionen:

```{python}
x.upper()
```

Im Beispiel oben wird ein neuer String erzeugt, welcher aus den Zeichen des Strings `x` in Großbuchstaben besteht.
...
Listen haben wir in der vorigen Einheit im Detail kennengelernt. Die Möglichkeit, Strings in einer Liste mit `join` zu einem einzigen String zu verbinden, ist übrigens wesentlich effizienter, als die einzelnen Strings mit `+` zu verbinden.
...
Wir haben bereits die `print`-Funktion kennengelernt, welche Strings (und andere Objekte) als Text am Bildschirm ausgeben kann. Mit der Funktion `str` kann man beliebige Objekte (also auch z.B. Ganzzahlen) in einen String "umwandeln":
...
Wie wir bereits wissen, kann man mehrere Strings mit `+` verketten. Daher könnte man obige Ausgabe auch durch Verketten von einzelnen Strings erzeugen:
...
Sogenannte f-Strings sind ganz normale Strings, man kann aber bei ihrer Erzeugung beliebige Ausdrücke verwenden, die dann ausgewertet in den String eingesetzt (interpoliert) werden. Man definiert einen f-String wie einen regulären String mittels Anführungszeichen, stellt aber ein `f` voran.
...
Ausdrücke wie z.B. Zahlen können optional auch *formatiert* werden. Mit einem optionalen Doppelpunkt und einem sogenannten *Format Specifier* innerhalb der geschwungenen Klammern kann ein Ausdruck gezielt formatiert werden. Das folgende Beispiel formatiert die Kreiszahl $\pi$ so, dass sie mit drei Nachkommastellen ausgegeben wird:
