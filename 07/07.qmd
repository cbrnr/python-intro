...
Wie im vorigen Kapitel eingeführt, werden Methoden direkt auf ein Objekt angewendet. Für Strings gibt es in Python sehr viele praktische Methoden:
...
::: callout-tip
Da viele Methoden einen neuen String zurückgeben, kann man mehrere Methodenaufrufe auch direkt hintereinander schreiben:

```{python}
x = "abcde"
x.upper().replace("A", "X").lower()
```

Die Methoden werden dabei von links nach rechts aufgerufen, d.h. zuerst `upper`, dann `replace` und zuletzt `lower`.
:::
...
Die Lösungen zu diesen Übungen finden Sie [hier](07-solutions.qmd).
