...
[PsychoPy](https://www.psychopy.org/) ist eine freie, in Python geschriebene Software zum Erstellen und Durchführen von Experimenten, insbesondere in der experimentellen Psychologie und den Neurowissenschaften. Es unterstützt zwei Arten um Experimente zu erstellen:

1. Visuelle Erstellung über eine grafische Oberfläche namens *Builder*
2. Programmatische Erstellung mit Python-Code (z.B. mit dem mitgelieferten Editor namens *Coder*)

Der Builder erleichtert den Einstieg, weil dafür zunächst kein Programmierwissen nötig ist. PsychoPy übersetzt ein damit erstelltes Experiment beim Ausführen automatisch in Python-Code. Viele Parameter lassen sich grafisch einstellen; bei komplexeren Designs ist direkter Python-Code jedoch oft flexibler. Man kann die grafische Oberfläche vollständig umgehen oder ein Builder-Experiment mit eigener Code-Komponente erweitern. Dafür sind grundlegende Python-Kenntnisse und ein Verständnis der PsychoPy-Abläufe nötig.
...
PsychoPy ist zwar ein Python-Paket und kann prinzipiell mit `pip` installiert werden. Wegen der zahlreichen, teils systemspezifischen Abhängigkeiten ist eine manuelle Installation aber fehleranfällig.

Die [offizielle Installationsanleitung](https://www.psychopy.org/download.html) beschreibt die Möglichkeiten für die einzelnen Betriebssysteme. Für diese Lehrveranstaltung sind die vorgefertigten [Standalone-Versionen](https://github.com/psychopy/psychopy/releases) die verlässlichste Option. Sie enthalten eine passende Python-Umgebung mit allen benötigten Zusatzpaketen und werden getrennt von einer eventuell bereits vorhandenen Python-Installation eingerichtet.
...
Die Versuchspersonen haben damit 2 Sekunden Zeit, eine der drei Tasten <kbd>r</kbd>, <kbd>g</kbd> oder <kbd>b</kbd> zu drücken. Falls keine Antwort erfolgt, endet die Routine und der nächste Schleifendurchlauf beginnt. Der Text-Stimulus wird nach 1 Sekunde ausgeblendet; während der zweiten Sekunde bleibt das Fenster leer, die Tastatur bleibt aber aktiv. Außerdem wird gespeichert, ob die Antwort korrekt war. Dazu verwenden wir die Spalte `correct` aus der Excel-Datei, die bereits die erwartete Taste enthält. PsychoPy vergleicht diese mit der tatsächlich gedrückten Taste und schreibt das Ergebnis in die Ergebnisdatei. Speichern Sie nur den ersten Tastendruck ("first key"), weil nur dieser als Antwort zählen soll.
...
Man könnte nun beispielsweise die mittleren Reaktionszeiten (Spalte `trial_key.rt`) der korrekt gelösten Trials (Spalte `trial_key.corr`) vergleichen: zwischen kongruenten Trials, bei denen Wort und Farbe übereinstimmen, und inkongruenten Trials, bei denen sie nicht übereinstimmen. Trials ohne Antwort sollten dabei ausgeschlossen werden.
...
Um nach jedem Trial rückzumelden, ob die Versuchsperson die richtige Taste gedrückt hat, fügen wir direkt nach der Routine "trial" eine neue Routine namens "feedback" hinzu. Sie muss *innerhalb der Schleife* liegen, damit das Feedback nach jedem Trial erscheint. Fügen Sie einen Text-Stimulus namens `feedback_text` hinzu. Weil dessen Inhalt vom vorherigen Trial abhängt, tragen wir bei Text `$msg` und im Tab "Appearance" bei der Textfarbe `$msg_color` ein. Diese beiden Python-Objekte enthalten später den Text ("Correct!", "Wrong!" oder "Too slow!") und die zugehörige Farbe (grün, rot oder weiß). Bei beiden Feldern muss "set every repeat" gesetzt sein. Setzen Sie außerdem die Stoppzeit auf 1 Sekunde.
...
Sehen wir uns an, wie dieser Code funktioniert. Wir verwenden hier die Keyboard-Komponente `trial_key`, welche zwei wichtige Attribute enthält:

- `trial_key.keys` enthält bei der Einstellung "first key" den Namen der ersten gedrückten Taste als Zeichenkette, beispielsweise `"r"`. Ohne Antwort ist der Wert leer bzw. nicht gesetzt.
- `trial_key.corr` enthält `1` für eine korrekte und `0` für eine inkorrekte oder fehlende Antwort.

Die Bedingung `if trial_key.keys` überprüft, ob eine Taste gespeichert wurde. Ein nicht-leerer Text entspricht in einer Bedingung dem `bool`-Wert `True`; ein leerer bzw. nicht gesetzter Wert entspricht `False`.
...
Wenn also eine Taste gespeichert wurde, wird der erste Codezweig ausgeführt. Dieser enthält die weitere Bedingung `if trial_key.corr`; `1` wird in einer Bedingung als `True` und `0` als `False` behandelt. Bei einer korrekten Antwort werden daher `msg` auf `"Correct!"` und `msg_color` auf `"green"` gesetzt. Bei einer inkorrekten Antwort wird der `else`-Zweig ausgeführt (`msg = "Wrong!"` und `msg_color = "red"`).
...
Die 20 Bedingungen sollen in zufälliger Reihenfolge angezeigt und dreimal wiederholt werden, d.h. insgesamt soll das Experiment aus 60 Trials bestehen.
...
Die Lösungen zu diesen Übungen finden Sie [hier](11-solutions.qmd).
